    ...
    if(event.type == "click") {
      ...
      return(
        <Container className="sendSignContainer">
          <p className="sendSignHeader">We are constantly working to add to our dictionary</p>
          <p className="sendSignHeader">We would love to help answer your question and get your sign added for future users! </p>
          <div className="submitSignEmailContainer">
            <input type="email" className="sendSignEmail" placeholder="Enter your email"/>
          </div>
          ...
          <div className="submitSignRecord">
            {
              ...
              "":
                  ...
                  <button className="submitSignButton" onClick={(event) => hideSubmitContainer(event)}>No</button>
                  <LinkContainer to="/confirmSubmit">
                    <button className="submitSignButton">Yes</button>
                  </LinkContainer>
                </div>
            }
            <svg role="button" onClick={(event) => hideSubmitContainer(event)} 
                className="sendSignRecord" id="Layer_1" 
                data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" 
                viewBox="0 0 18.16 18.16">
                <circle style={{fill:'#cae7eb'}} cx="9.08" cy="9.08" r="8.19"/>
                <path style={{fill:'#0e4e7c'}}
